...
const getTitle = computed(() =>
  data.value?.create
    ? $t('ui.modal.add', { moduleName: $t('page.material.audio.module') })
    : $t('ui.modal.update', { moduleName: $t('page.material.audio.module') }),
);
...
const [BaseForm, baseFormApi] = useVbenForm({
  ...
  schema: [
    ...
    },
    {
      ...
      fieldName: 'resource',
      label: $t('page.material.audio.resource'),
      help: '支持 MP3、WAV、AAC 等格式，大小不超过 10MB',
      componentProps: {
        type: 'audio',
        limit: 1,
        maxSize: 10,
    ...
    },
    {
      ...
      label: $t('page.material.audio.description'),
      defaultValue: '',
      componentProps: {
        type: 'textarea',
...
const [Drawer, drawerApi] = useVbenDrawer({
  ...
  onOpenChange(isOpen) {
    if (isOpen) {
      ...
      // 如果是编辑模式且有音频 URL，转换为数组格式供 ImageUpload 显示
