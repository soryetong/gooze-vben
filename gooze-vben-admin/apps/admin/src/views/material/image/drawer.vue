...
const getTitle = computed(() =>
  data.value?.create
    ? $t('ui.modal.add', { moduleName: $t('page.material.image.module') })
    : $t('ui.modal.update', { moduleName: $t('page.material.image.module') }),
);
  ...
  schema: [
    {
      component: 'Input',
      fieldName: 'name',
      label: $t('page.material.image.name'),
      componentProps: {
    ...
    },
    {
      ...
      fieldName: 'resource',
      label: $t('page.material.image.resource'),
      help: '支持 JPG、PNG 格式，大小不超过 500KB',
      componentProps: {
        type: 'image',
        limit: 1,
        maxSize: 800,
    ...
    },
    {
      ...
      label: $t('page.material.image.description'),
      defaultValue: '',
      componentProps: {
        type: 'textarea',
...
const [Drawer, drawerApi] = useVbenDrawer({
  ...
  onOpenChange(isOpen) {
    if (isOpen) {
      ...
      // 如果是编辑模式且有图片 URL，转换为数组格式供 ImageUpload 显示
